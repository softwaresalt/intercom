...
| SQ-a -- Abort vs. blocked handler | **ANSWERED: NO** | `Session.Abort` returned promptly **without** unblocking an already-blocked `PermissionHandlerFunc` (handler remained parked on an unclosed channel after `Abort` returned). `PermissionHandlerFunc`'s signature carries no `context.Context`, so `Abort` has no channel through which to deliver a cancellation signal into an in-flight handler call. **Confirms** design section 3.1's conservative assumption ("until answered, assume it does not"). |
