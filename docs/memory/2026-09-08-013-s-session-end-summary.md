...
- Post-merge closure artifact: `docs/closure/013-S-014-F-post-merge-closure.md` (renamed from `docs/closure/2026-09-08-013-s-014-f-pathsafe-reparse-containment-closure.md` to the canonical `{shipment_id}-{feature_id}-post-merge-closure.md` naming the `pipeline-topology` gate's `closure_complete()` reader requires — repaired post-merge, same defect class as 012-S/PR #39) — releasability `READY_WITH_CONDITIONS` (frontmatter added per Copilot review in PR #41).
