...
| ID | Measured fact | Evidence |
|---|---|---|
| **D-C11** | `shipment-reconcile` persists a report at **`.backlogit/reconcile/{shipment_id}-{mode}-{timestamp}.md`** for the modes that persist one — `pre`, `safe-close`/`cascade-close`, and `post`. **`classify-close-path` persists nothing under any invocation** (D-C16), so "every mode" would overstate the tool and make P4 unsatisfiable if read literally. | `SKILL.md` L65, L321; persistence clause |
