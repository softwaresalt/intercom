...
| **D-A7** | **The route declares an explicit safety mode before it mutates anything (Constitution Principle VIII).** S-0 declares **careful + freeze-scope**: every destructive or irreversible step (S-3 claim, S-18 cascade, R-3…R-5) is enumerated with its approval path before execution, and edits are frozen to the §H permitted surfaces. The declaration is recorded in `{run_log}`. Omitting it is a Principle VIII violation by omission. |
...
The canonical measurement table is **plan §3 (`M-1…M-29`)** — one table, one identifier system.
Decisions below cite `M-n` directly. Measurements are observed facts, not decisions: they are
recorded once, in the executable artifact that consumes them, and are never restated here under a
second set of IDs. Every row is measured against the **installed** workspace; where the `021-S`
closure of 2026-09-16 supplies empirical evidence it is cited there (cascade commit `17f891f`,
closure merge `f2d4cf9`, PR #56).

Two measured facts carry no standalone `M-` row because the plan states them at the point where
they bind: `abandoned` is terminal, destructive, **not** covered by `PA-017-CASCADE` and carries no
operator approval (plan §8, forbidden-exit note); the `harness-architect` producer **is installed**
at `.github/skills/harness-architect/` (plan §4.2, citing **M-29**).

The **disposition** of the P-002 ordering conflict is a decision, not a measurement — see **D-D6**.

## D. Close-path authority

| ID | Decision |
|---|---|
| **D-D1** | Installed Ship requires the returned `CLASSIFICATION_BINDING` to be carried into the close call. By M-15 the direct cascade op accepts no binding. Therefore **`mode: safe-close` carrying a `CASCADE` binding is the only executable realization of installed Ship's own mandate**, not an alternative to it. The same `backlogit shipment ship` operation runs either way. |
...
| **D-D6** | **P-002 is DISCLOSED AS UNSATISFIABLE and requires an EXPLICIT RECORDED OPERATOR AUTHORIZATION to proceed — disclosure alone is NOT sufficient.** Per M-29, the shipment claim at S-3 auto-activates `018.008-T`, so P-002's "task claim after `harness-ready`" ordering cannot hold on any shipment-claim route. **P-002's Violation Action is literally "Halt and suggest running the harness-architect."** No disposition in the installed registry authorizes proceeding past a Halt-mandated gate on notification; every override this registry defines is an explicit recorded authorization (P-001 `skip_policy`, P-014 merge approval, P-018 audited force, P-012 declared degradation, `PA-017-CASCADE` escrow). This plan therefore: (1) states the deviation explicitly rather than omitting the policy; (2) surfaces it to the **operator before S-3** and **requires an explicit recorded operator authorization to proceed**, captured verbatim in the run log — **absence, ambiguity, or silence ⇒ HALT to Stage, zero mutation (Regime A)**; the authorization reuses the existing `skip_policy: P-002`-shaped acknowledgement rather than minting a new token; (3) emits a P-005 record per M-22 with `violation_policy: P-002`, `gate: S-3`, `skip_policy: P-002`; (4) preserves P-002's **protective intent** via D-D7, not via a self-applied label; (5) does **not** amend P-002 and does **not** assert conformance. The general conflict is captured as stash **`DB12DA37`** and is out of scope (D-M8). |
| **D-D7** | **P-004's PRODUCER CONTRACT IS NOT DEVIATED FROM. The `harness-ready` label is applied ONLY by the installed `harness-architect` skill.** P-004's Violation Action is literally "Do NOT apply `harness-ready` label", and P-002 names the harness-architect as the **producer** (Ship is only the consumer). Ship self-applying the label would emit a **false compliance signal on the exact label P-002 consumes** — worse than omitting it. S-7 therefore **invokes the installed `harness-architect` skill** for `018.008-T`; the skill applies the label at its own Step 6 after its Step 5.1/5.2 checks. The pass criterion adopts P-004's precondition **and** `018.008-T`'s recorded AC-13 literals verbatim: `go vet ./...` exits 0; `go test ./...` exits non-zero (P-004's own literal precondition); `go test ./tests/integration/ -run TestStageBranchGate_ContractCorrected` exits non-zero (the AC-13 literal — equivalent here, because this release unit generates exactly one failing function, and the scoped run is what pins the red to that function rather than to unrelated breakage); the captured output contains the literal marker `not implemented: stage branch gate contract` attributed to that function; the H0 red count is literally 1 of 1; and the harness manifest records `Compilation: PASS` and `Red Phase: CONFIRMED`. Any shortfall ⇒ **do not apply the label** and HALT (Regime B). Only P-002's *ordering* is deviated from (D-D6); P-004 is satisfied in full. |
| **D-D8** | **The green phase is as literal as the red phase (Constitution Principle II, Quality Gates).** S-7 establishes red under **six** literals; S-8 must establish green under an equally explicit criterion, otherwise "implement to green" is unfalsifiable. S-8's pass criterion is: `gofmt -l .` produces **no output**; `go vet ./...` exits **0**; `go test ./...` exits **0**; `go build ./...` exits **0**; and `TestStageBranchGate_ContractCorrected` is reported **PASS** (the exact function whose red S-7 recorded). All five are recorded in `{run_log}`. Any shortfall ⇒ **HALT to the operator (Regime B)** — S-8 is post-claim. |
| **D-D9** | **PF-4 positively verifies `018-F` is a root, pre-claim.** P-015's fully-covered-root exception has root-ness as precondition 1, so the route may not merely assume it. PF-4 reads `018-F`'s raw frontmatter and requires that it declares **no `parent_id`** (absent, empty, or null) and that no other artifact declares `018-F` as a child of a further parent. Failure ⇒ halt to Stage with **zero mutation** (Regime A) — PF-4 is pre-claim. |
| **D-D10** | **Every mutating step states its exit contract.** S-9 (`018.008-T` → `done`) passes only when the transition command exits **0** and a **re-read** of the artifact confirms `status: done` at its post-transition path (M-17 relocates the record on a `→ done` transition, so the re-read follows the routed path, not the pre-transition one). Exit non-zero, or a re-read that does not confirm `done` ⇒ **HALT to the operator (Regime B)**. |

## E. Happy path (canonical sequence)

**Preflight gates `PF-1…PF-12`** run in order on the pre-claim branch, before the claim. Any
...
| **PF-12** | **Producer admission gate (P-012, read-only, PRE-CLAIM).** `.github/skills/harness-architect/SKILL.md` resolves, and its Step 1 intake admits a work item whose status is `active` — or a declared P-012 fallback exists. The §D P-002 mitigation rests entirely on this producer; validating it at S-7 would place the check **after** the S-3 one-way door, so a refusal would strand `017-S`. Log `TOOL_OK` / `TOOL_DEGRADED`. Failure ⇒ HALT to Stage, Regime A, zero mutation. |
...
| ID | Step | Binds |
|---|---|---|
| **S-0** | **P-002 deviation gate (pre-claim, before S-1).** Probe the broadcast surface (P-012); disclose the D-D6 deviation to the operator; emit the three-action P-005 record (M-22). **Pass criterion: an explicit recorded operator authorization to proceed, captured verbatim. Absence, ambiguity or silence ⇒ HALT to Stage, zero mutation (Regime A).** Mutates nothing. | — |
...
| **S-22** | Closure PR: P-014 local review gate, **P-018 Copilot-review gate**, D-I1 pre-merge merge-strategy verification, operator-approved **merge commit**, verified on `origin/main`. §H changed-path control asserted (D-H1 deny-list, D-H2 membership, P1-P6). | `{closure_merge_sha}` |

## F. Variable bindings (defined before first use)

| Variable | Bound at | First consumed at |
|---|---|---|
| `{run_log}` | **S-0** | **S-0** (safety-mode declaration, D-A7), then every subsequent step (D-G6) |
| `{harness_paths}` | **S-7** | **S-8** (union-cardinality assertion, D-H2) |
...
| **D-G6** | **`{run_log}` is a bound artifact, not a figure of speech.** It is the single append-only evidence file for this run, at `docs/plans/evidence/2026-09-16-017-S-closure/run-log.md`, created at **S-0** and committed at **S-21.5** alongside the closure artifacts. Every step that says "record in the run log" appends a timestamped entry naming the step ID, the command, its exit code, and the measured values. It is markdown under version control (Principle IX) and is retained as closure evidence, never pruned mid-run. Because it is **uncommitted and continuously growing from S-0 until S-21.5**, it is excluded from every path binding and every cleanliness gate by the canonical pathspec in **D-H6**, is carved out of the D-H1(e) deny-list, and is a member of the D-H5 S-22 expected-artifact set. S-21.5 commits it explicitly alongside the closure artifacts. |
| **D-G7** | **P-008 markdown lint has exactly two gate points.** `markdownlint` runs over every markdown path this route writes: **before the S-8 commit** (the policy and agent-surface edits) and **before the S-21.5 commit** (the closure artifacts and `{run_log}`). Non-zero output ⇒ fix and re-run; it is a pre-commit quality gate, not a post-hoc observation. Both gate points are pre-commit, so a lint failure is always recoverable in place and never strands the shipment. |

## H. Changed-path control (deny-list + positive invariants)

**Design rule.** Changed-path control is a **deny-list plus positive invariants**, never an
exhaustive allowlist. An exhaustive allowlist (`anything not enumerated ⇒ HALT`) requires
perfectly predicting every path a multi-tool chain touches, and its failure mode is catastrophic:
a halt **after** the one-way door, stranding `017-S` `active` with no unclaim. The deny-list form
is robust to unanticipated tool emissions because an unforeseen artifact is not automatically
fatal.

| ID | Decision |
|---|---|
| **D-H1** | **FORBIDDEN SURFACES (deny-list).** A changed path is a violation if and only if it matches one of: (a) production implementation code — `internal/**`, `cmd/**`, any `*.go` that is not a `*_test.go` file, `go.mod`, `go.sum` (the `*_test.go` carve-out applies uniformly, including under `internal/` and `cmd/`, though `018.008-T`'s recorded scope independently forbids any harness there); (b) `.github/policies/**`, `.github/agents/**`, `.github/skills/**`, `.github/instructions/**` or `.github/workflows/**` **other than** the two surfaces `018.008-T`'s recorded scope authorizes (see D-H2); (c) any `.backlogit/queue/**` or `.backlogit/archive/**` record **not** belonging to the 13 manifest members, the `017-S` record, or the evidenced `019.007-T` delta (D-H4); (d) any deletion of a `.backlogit/reconcile/**` report; (e) **governance and control-plane surfaces** — `docs/plans/**` and `docs/decisions/**` (including this contract, the governing closure plan, and the FROZEN prerequisite `docs/plans/2026-09-13-intercom-go-ship-feature-completion-decided-plan.md`), `.autoharness/**`, `.backlogit/registry.yaml`, `.backlogit/header-def.yaml`, `.backlogit/config.yaml`, `scripts/**`, `.gitignore`, `go.work*`. **CARVE-OUT to (e):** this run's own evidence directory `docs/plans/evidence/2026-09-16-017-S-closure/**` is **NOT denied**, notwithstanding its `docs/plans/**` prefix. `{run_log}` (D-G6) lives there and D-G6 requires it committed at S-21.5, so without this carve-out the S-22 closure-PR diff would contain a deny match and halt **after** the irreversible cascade has landed — the precise post-claim stranding §H exists to eliminate. The carve-out is scoped to this one directory; all other `docs/plans/**` paths stay denied. Nothing else is fatal by enumeration. **Rationale for (e):** a deny-list is fail-open by design, so every surface whose immutability this plan asserts elsewhere (§M out-of-scope list, I-7's FROZEN prerequisite, the routing table M-17 depends on) must be named here or that assertion has no path-level enforcement at all. |
...
| **D-H5** | **EXPECTED-ARTIFACT SETS, measured (M-20/M-21), used to satisfy P4 — never as a deny-list.** **Cascade commit (S-21):** backlog record changes (queue→archive renames, archive-side modifies) **and** the reconcile reports for the modes that persist one. **Closure PR (S-22):** the cascade commit plus `docs/closure/017-S-018-F-post-merge-closure.md`, `docs/memory/*.md`, `docs/compound/*.md`, **and `{run_log}` at `docs/plans/evidence/2026-09-16-017-S-closure/run-log.md`**, all authored or committed at S-21.5. **P4 is scoped to modes that PERSIST a report** — `pre`, `safe-close`/`cascade-close`, and `post`. Per M-19 the installed skill writes **no** report for `classify-close-path` under any invocation, so requiring one would make P4 unsatisfiable by construction. The measured report filenames are `017-S-pre-*.md`, `017-S-cascade-close-*.md` and `017-S-post-*.md`; a literal `{mode}` substitution would mis-predict the cascade filename. A path in these sets is **expected**; a path outside them is **reported and recorded** but is only fatal if it matches D-H1. |
| **D-H6** | **EVERY changed-path binding AND every worktree-cleanliness check excludes backlog state and this run's evidence directory.** The single canonical exclusion pathspec for this run is **`-- . ':(exclude).backlogit/' ':(exclude)docs/plans/evidence/2026-09-16-017-S-closure/'`**. Bind **both `{harness_paths}` (S-7) and `{impl_paths}` (S-8)** from `git status --porcelain=v1` under that pathspec, and apply the same pathspec to **every** `git status` cleanliness assertion in this run (S-1, S-12, R-5(1)). Two independent reasons, both fatal if unhandled: the S-3 claim writes `.backlogit/` records that no step has committed yet and S-2 writes a `.backlogit/reconcile/` report; and `{run_log}` is **appended continuously from S-0 and is uncommitted until S-21.5** (D-G6). An unfiltered `git status` would ingest either into a path binding — breaking D-H2's closed-set membership test and the `≤ 3` cardinality bound — and would make the S-12 and R-5(1) cleanliness gates fail deterministically **post-claim**. `.backlogit/**` is **outside D-H2's domain entirely** and is governed solely by P3 — a `.backlogit/` path is therefore never a D-H2 membership failure. |
...
| ID | Decision |
|---|---|
| **D-I1** | **P-009 is Merge-Commit-Only, and it is a PRE-MERGE gate.** Both the S-10 and S-22 merges MUST be true merge commits. Squash-merge and rebase-merge are **forbidden**. P-009's Gate Point is *"Ship Step 5 pre-merge (before any merge action)"* and Constitution Principle XI (NON-NEGOTIABLE) states *"The ship agent MUST verify the merge strategy **before executing any merge**"*. Therefore, **before** invoking any merge at S-10, S-22 or R-5(5), verify the repository merge configuration: `gh api repos/{owner}/{repo} --jq '{merge:.allow_merge_commit,squash:.allow_squash_merge,rebase:.allow_rebase_merge}'` MUST report `allow_merge_commit=true`; and the merge MUST be invoked with an explicit merge-commit method (`gh pr merge --merge`), never `--squash` or `--rebase`. Any other configuration or method ⇒ **P-009 violation, HALT before merging, broadcast P-005**, per the policy's Violation Action. **Post-hoc detection is not a substitute:** D-I2's parent-shape check runs *after* the merge has landed on `main`, and an incorrectly-shaped merge cannot be undone without history rewriting, which Principles VII and XI forbid. D-I2 is confirmation, not the gate. |
| **D-I2** | S-11 **confirms** the S-10 merge empirically after it lands (the pre-merge gate is D-I1): **exactly two parents**, parent 1 = mainline, parent 2 = the reviewed PR head. Any other shape is a **P-009 violation ⇒ halt**. |
...
| **D-J4** | In Regime B, **forbidden**: `return-blocked` (M-11), `move` on the shipment (M-12), `abandoned` (terminal, destructive, uncovered by `PA-017-CASCADE`, unapproved — §C), `backlogit move <id> --status archived` on any member (a **silent no-op**, not an archive operation — M-13), and R-1…R-8 (they presuppose cascade effects and a `{pre_cascade_sha}` anchor). |
...
| **D-J9** | **Violation telemetry — all THREE P-005 required actions, not one.** Every halt corresponding to a named policy performs the full installed P-005 triple: (1) **broadcast** the violation with policy ID and one-line summary; (2) **include the violation details in the PR description as a compliance annotation** — marked `N/A (no PR exists at this gate)` when the halt precedes PR creation, per P-021 C3's per-field availability precedent; (3) **record it in the memory checkpoint** for the affected artifacts. Each event records the policy ID, failure token, step ID and affected artifact IDs. Recording action (1) or (3) alone is a P-005 shortfall and must not be certified as acceptance-passing. |
| **D-J10** | **P-021 disposition:** findings raised during S-8 implementation or the S-10 review cycle are classified C1 (in-scope, fix now), C2 (out-of-scope ⇒ capture as a deferred stash entry with source refs), or C3 (defect in this plan ⇒ **HALT to the OPERATOR, Regime B (D-J11)** — S-8 and S-10 are both post-claim, so Stage is never the halt target; Stage re-measurement is advisory input to the operator only). Scope is never widened in place. |
| **D-J11** | **Halt routing follows the regime, never the authoring agent.** A halt is routed to **Stage** only under Regime A (`017-S` NOT `active`). **Every halt at or after S-3 routes to the OPERATOR (Regime B)**, including halts inside steps Stage authored (S-4, S-7) and every risk-register mitigation. Routing a post-one-way-door halt to Stage hands a stranded `active` shipment to the one agent P-010 forbids from disposing of shipments. Stage re-measurement may be advisory input to the operator; it is never the halt target. **Installed-wording precedence.** Where an installed agent surface phrases a post-claim halt as returning to Stage — as `_ship.agent.md`'s Step 6.1(a1) `S5` branch does — this plan does **not** amend the installed text and quotes it verbatim. The **disposition** is nonetheless governed by this row: the halt is surfaced to the **operator**, and any Stage involvement is advisory re-measurement only. |
| **D-J12** | **P-020 compaction failure is NON-BLOCKING** (installed P-020 Violation Action, verbatim): a compact-context run that FAILS records `compaction: degraded` in the closure artifact, logs a warning, and **continues closure** — the merge has already landed. Only **SKIPPING** the invocation is the violation, recorded via the full P-005 triple with `violation_policy: P-020`, `gate: Ship Step 6 closure`, `action: closure-incomplete`. S-21.5 is therefore explicitly **excluded from D-J1's fail-closed default and from every R-trigger**; treating a failed compaction as a halt would invert the installed policy after the merge has landed. |
| **D-J13** | **P-013 circuit state is recorded, not re-derived.** This plan's review lineage has crossed the P-013.3 consecutive-failure threshold. The plan records the P-013 event through the full P-005 triple, names the resolved P-013.6 escalation route (or `ESCALATION_DEGRADED`), and carries a **P-013** row in the constitution map. Each re-attempt past the open circuit proceeds **only** on an explicit recorded operator authorization — the agent never self-authorizes one (P-013.6 authority-preservation invariant). |
| **D-J14** | **A halt NEVER destroys evidence (Constitution Principle VII).** No halt disposition anywhere in this route discards, resets, cleans, or force-checks-out a working tree, branch, or commit. Work in progress at a halt is **committed on its own branch and left intact** so the operator inherits the full red/green evidence trail. Discarding a working tree is a destructive act requiring explicit approval, and at a post-claim halt it would destroy the exact S-7 red-phase evidence P-004 exists to produce. |
...
| **D-K7** | **R-5 Committed, closure merge landed. The revert target is `{cascade_commit_sha}`, NEVER `{closure_merge_sha}`.** The closure branch carries **three** commits: the S-15 pre-cascade baseline `{pre_cascade_sha}` (which contains S-14's `018-F → done`), the S-21 cascade commit `{cascade_commit_sha}`, and the S-21.5 closure-documentation commit. `{closure_merge_sha}` contains **all three**, so `git revert -m 1 {closure_merge_sha}` would unwind the baseline and the closure documentation as well as the cascade — returning `018-F` to `active`/queue-located and making D-K9/D-K10 **unsatisfiable by construction**, since `{pre_paths}` was captured *after* the baseline commit. R-5 therefore: (1) **P-011/P-016 prechecks first** — `git status --short` **under the canonical exclusion pathspec (D-H6)** clean and `git worktree list --porcelain` classified fail-closed, **before any branch creation**; (2) verify `{closure_merge_sha}` empirically as **evidence only**: exactly two parents, parent 1 = mainline, parent 2 = the reviewed closure-PR head by **SHA equality**; any other shape ⇒ P-009 violation, HALT to operator, no automatic revert; (3) **execution site (P-010): never on `main`** — `git checkout main && git pull && git checkout -b chore/017-s-cascade-revert-<utc>`; (4) `git revert --no-edit {cascade_commit_sha}` **on that branch** — `{cascade_commit_sha}` is single-parent (M-25), so **plain revert, never `-m 1`**; (5) land it through its own revert PR under the full gate set — P-014, P-018, explicit operator approval, **P-009 merge commit** — binding `{revert_merge_sha}`; (6) D-K9/D-K10 are evaluated **against `origin/main` at `{revert_merge_sha}`**, never against an unmerged branch; (7) any failure at (5) ⇒ HALT to operator; Ship never lands the revert by any other path. **This limb lands a merge on `main` under a new explicit operator approval — it is a moderate-to-high risk action, not a low additive one.** |
...
| **D-K12** | Post-restore the run **HALTS to the OPERATOR** with the R-6/R-7 inventories and `{pre_cascade_sha}`: the unwind succeeds against the tree, but `017-S` is still `active` and its only exits are `shipped`/`abandoned`, so disposition is the operator's (D-J11) and never Stage's (P-010). Stage re-measurement is advisory input only. `PA-017-CASCADE` is not re-exercised in the same session. |
