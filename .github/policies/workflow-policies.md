...
- Create and check out a dedicated Stage artifact branch (`chore/stage-{scope-slug}`)
- Commit backlog and planning artifacts on that dedicated Stage artifact branch only — never on the default branch
...
A Stage artifact branch or its PR carries no source, test, or config change; it is therefore neither an implementation branch under P-016 nor a release unit under P-001. This clarification does not grant, imply, or restate any Orchestrator authority to commit Stage artifacts or push the default branch; the Orchestrator's role statement above is unchanged.
...
| 1.25.0  | 2026-09-18     | Corrected P-010  | 017-S: Stage's commit grant is now dedicated-Stage-artifact-branch-only (`chore/stage-{scope-slug}`) — the prior "default branch or a dedicated chore/admin branch" grant is withdrawn; added an actor-named Stage MUST NOT bullet forbidding direct commit/push to `main`, symmetric with Ship's existing bullet; added an explicit Stage MAY grant to create and check out `chore/stage-{scope-slug}`; and added a clarifying sentence that a Stage artifact branch/PR carries no source/test/config change and is therefore neither a P-016 implementation branch nor a P-001 release unit, without granting or restating any Orchestrator authority to commit Stage artifacts or push the default branch. The P-016 spike/research exception paragraph is unchanged |
